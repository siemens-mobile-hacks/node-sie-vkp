...
export as namespace moo;
	...
	export function compile(rules: Rules | RulesArray): Lexer;

	export function states(states: { [x: string]: Rules }, start?: string): Lexer;

	export interface Rule {
		match?: RegExp | RegExp[] | string | string[] | undefined;
		/**
		 ...
		 */
		type?: TypeMapper | string | undefined;
	}

	type RuleDefinition = RegExp | RegExp[] | string | string[] | Rule | Rule[] | ErrorRule | FallbackRule;

	export interface Rules {
		[x: string]: RuleDefinition;
	}

	export type RulesArray = RuleDefinition[];
